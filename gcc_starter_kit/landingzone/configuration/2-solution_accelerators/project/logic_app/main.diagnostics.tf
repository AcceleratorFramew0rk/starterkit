module "diagnosticsetting1" {
  ...
  name                = "${module.naming.monitor_diagnostic_setting.name_unique}-logicapp"
  target_resource_id = module.logicapp.resource.id
  ...
  diagnostics = {
    categories = {
      log = [
        ...
        ["WorkflowRuntime", true, false, 7],
        ["FunctionAppLogs", true, false, 0],
