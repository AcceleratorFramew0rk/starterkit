...
module "firewall" {
  ...
  firewall_ip_configuration = [
    {
      name                 = "ipconfig1"
      subnet_id            = try(local.remote.networking.virtual_networks.hub_intranet_egress.virtual_subnets.subnets["AzureFirewallSubnet"].id, null) != null ? local.remote.networking.virtual_networks.hub_intranet_egress.virtual_subnets.subnets["AzureFirewallSubnet"].id : var.subnet_id 
  ...
  firewall_management_ip_configuration = {
    name                 = "ipconfig2"
    # subnet_id            = local.remote.networking.virtual_networks.hub_intranet_egress.virtual_subnets.subnets["AzureFirewallManagementSubnet"].id # module.virtualnetwork_ingress_egress.subnets["AzureFirewallManagementSubnet"].id  # azurerm_subnet.subnet.id
    subnet_id            = try(local.remote.networking.virtual_networks.hub_intranet_egress.virtual_subnets.subnets["AzureFirewallManagementSubnet"].id, null) != null ? local.remote.networking.virtual_networks.hub_intranet_egress.virtual_subnets.subnets["AzureFirewallManagementSubnet"].id : var.azurefirewallmanagement_subnet_id 
